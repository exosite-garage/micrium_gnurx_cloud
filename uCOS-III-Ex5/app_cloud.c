...
extern CPU_INT16S  AppTempSensorDegC;
...
static void CloudData_Task (void *p_arg)
{
    ...
    // OS Name = "Micrium-Ex5" <- MAX Length = 24
    // OS Ver  = "3.01.2" <- MAX Length = 8
    // Use network interface '1' MAC address
    cloud_available = Exosite_Init("renesas", "Micrium-Ex5", "3.01.2", (NET_IF_NBR)1);
    ...
    while (DEF_TRUE)
    {
        ...
        else
        {
            if (read_count++ > 10) {
                ...
                // Create string from temp value
                Str_FmtNbr_Int32U(AppTempSensorDegC,
                                  3u,
                                  DEF_NBR_BASE_DEC,
                                  ASCII_CHAR_NULL,
                                  DEF_NO,
